...
export function updatePlaceholder() {
    const platformSelect = getElement('platform-select');
...
function bindHistoryEvents() {
    ...
    const historyEl = getElement('history-dropdown');
    if (!historyEl) return;
...
function updateHistoryDropdownPosition() {
    ...
    try {
        ...
        const rightOffset = Math.max(
            0,
            Math.round(containerRect.right - buttonRect.left + APP_CONFIG.HISTORY.DROPDOWN_RIGHT_GAP)
        );
...
// All exported functions are automatically available on window object
